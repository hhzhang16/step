...
public final class FindMeetingQueryTest {
  ...
  public void optionalAttendeeCannotAttend() {
    // The required attendees have different events, while an optional attendee has an all-day event. 
    // We should see three options because there is no time that can fit all attendees, but
    // each required attendee has split the restricted times.
    //
    // Events  : |--------------C--------------|
    //                 |--A--|     |--B--|
    // Day     : |-----------------------------|
  ...
  @Test
  public void everyAttendeeIsConsideredIncludingOptional() {
    // Have each person have different events. We should see two options that fit the restricted times.
    //
    // Events  :       |--A--|--C--|--B--|
    // Day     : |-----------------------------|
    // Options : |--1--|     |--2--|     |--3--|
  ...
  @Test
  public void onlyOptionalSeveralGaps() {
  ...
  public void justEnoughRoomIgnoreOptional() {
    ...
    // have the meeting if the optional attendee is ignored.
    //
    // Events  : |--A--|-C-| |----A----|
    // Day     : |---------------------|
    // Options :       |-----|
    ...
    Collection<Event> events = Arrays.asList(
        new Event("Event 1", TimeRange.fromStartEnd(TimeRange.START_OF_DAY, TIME_0830AM, false),
            Arrays.asList(PERSON_A)),
        new Event("Event 1", TimeRange.fromStartDuration(TIME_0830AM, DURATION_15_MINUTES),
            Arrays.asList(PERSON_C)),
        new Event("Event 2", TimeRange.fromStartEnd(TIME_0900AM, TimeRange.END_OF_DAY, true),
            Arrays.asList(PERSON_A)));
  ...
  @Test
  public void notEnoughRoomOnlyOptional() {
    // Have two optional people, but make it so that there is not enough room at any point in 
